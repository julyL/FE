...
if (!Array.prototype.filter){
    Array.prototype.filter = function(fun /*, thisArg */){
        ...
        if (this === void 0 || this === null)
            throw new TypeError();
        var t = Object(this);          // 用于将普通类型转换为对象(Number,Boolean等),使得t.length不会报错,更加健壮
        var len = t.length >>> 0;      // t.length==整数取?t.length:0  (t.length为负数时,会有问题)
        if (typeof fun !== "function")
            throw new TypeError();
        var res = [];
        var thisArg = arguments.length >= 2 ? arguments[1] : void 0;
        for (var i = 0; i < len; i++){
            if (i in t){     //去掉未定义的部分  eg:  var arr=[];  arr[5]=1;  ==>  [undefined*4,1]   前4个均未定义
                var val = t[i];
